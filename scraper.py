...
def stonks():
    ...
    with webdriver.Firefox(options=options) as driver:
        ...
        try:
            ...
            os.system("git add" + " stonks/temp/all_" + date + ".csv")
            os.system("git add" + " stonks/temp/reduced_" + date + ".csv")
...
# creates daily file
for day_iter in ['04_21', '05_18', '05_19', '05_20', '05_21', '05_25', '05_26', '05_27', '05_28', '05_31', '06_01', '06_02', '06_03', '06_04']:
    ...
    for hour in ["09_30", "10_30", "11_30", "12_30", "13_30", "14_30", "15_30", "16_30", "17_30", "18_30"]:
        hour_df = pd.read_csv("stonks/temp/reduced_"+ year + "_" + f"{month:02}" + "_" + f"{day_iter:02}"[-2:] + "_" + hour + ".csv")
        hour_df.columns = ['accion', 'ultima_cotizacion', 'max_sesion', 'min_sesion', 'ultima_actualizacion']
        hour_df['fecha'] = str(year+"/"+ f"{month:02}" + "/" + f"{day_iter:02}"[-2:] )
        hour_df['hora'] = str(hour)
        #print(hour_df)
        day_df = day_df.append(hour_df, ignore_index=True)
    day_df.to_csv("stonks/" + year + "_" + f"{day_iter:02}" + ".csv", index=False, header=False)
